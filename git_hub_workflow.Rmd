...
## Branches
When several people are working on the same code base, it's sometimes useful to share your work and changes with others before they get merged into the "master" repository. Or you want to make some changes to the code, while at the same time still work with the existing codebase on some other aspect.

For this we use [**branches**](https://git-scm.com/book/en/v2/Git-Branching-Branches-in-a-Nutshell). A branch gives the freedom to work with the code base and try out things without affecting any other part. With the option to merge it back into the "master" branch. Branches can easily created from the command line using `git checkout -b <branch name>` or in RStudio.
![RStudio branches](images/branches_rstudio.png)

We can then commit the changes and push the branch to the original repository and issue a pull request. Others can then download the branch, test if the changes work for them, add to the branch. When all changes look good, the bransh can be merged back into the "master" branch.

It's almost always a good idea to make changes in a branch, even if the intention is to merge the changes into master right away.
